...
class DefaultController extends Controller {
    ...
    public function actionIndex() {
        ...
        $sql = "select a.id,a.screen,a.piece_id,a.pos_x,a.pos_y,a.dim_x,a.dim_y,a.align,c.name as layertype,a.element_id as table_ref,(CASE WHEN b.media IS NULL OR b.media = '' THEN d.name ELSE b.media END) as table_source,b.media,d.name, 
        ...
        foreach ($reader as $row) {
            ...
            try {
                ...
                if ($src == 'morpheme') {
                    ...
                    $subsql = "SELECT a.name,b.name as idiom FROM morpheme a join idiom b on(a.idiom_id=b.id) where a.id='$row[table_ref]'";
                    ...
                    $subsql = "SELECT a.name,b.name as idiom FROM phrase a join idiom b on(a.idiom_id=b.id) where a.id='$row[table_ref]'";
                    $command = Yii::app()->db2->createCommand($subsql);
                    $command->execute();
                    $item = $command->queryRow();
                    ...
                    $subsql = "SELECT a.name,b.name as idiom FROM paragraph a join idiom b on(a.idiom_id=b.id) where a.id='$row[table_ref]'";
                    $command = Yii::app()->db2->createCommand($subsql);
                    $command->execute();
                    $item = $command->queryRow();
                    ...
                    $subsql = "SELECT a.name,b.name as idiom FROM word a join idiom b on(a.idiom_id=b.id) where a.id='$row[table_ref]'";
                    $command = Yii::app()->db2->createCommand($subsql);
                    $command->execute();
                    $item = $command->queryRow();
                    ...
                    $subsql = "SELECT a.*,b.name as nstyle FROM image a join style b on(b.id=a.style) where a.id='$row[table_ref]'";
                    $command = Yii::app()->db2->createCommand($subsql);
                    $command->execute();
                    $item = $command->queryRow();
                    ...
                    $libraryProperty = new LibraryProperty();
                    $libproperty = CommonProperty::model()->findByAttributes(array('context' => 'library', 'name' => 'style'));
                    $libraryProperty->libraryID = $library->ID;
                    $libraryProperty->propertyID = $libproperty->ID;
